...
public:
	...
	void findSolution() {
		...
		if (total_rolls == 1) {
			...
			optimalSolution.change_best_stakes(dynamic_solution_start, ((((double)local_parameters.board_hits / (double)local_parameters.board_size)) * (local_parameters.payout_factor * local_parameters.max_bet)));
			return;
		}
